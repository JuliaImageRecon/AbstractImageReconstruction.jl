...
# ## Internal Interface
# Reconstruction algorithms in AbstractImageReconstruction.jl are expected to be implemented in terms of distinct processing steps, implemented in their own `process` methods.
# The `process` function takes an algorithm, parameters, and inputs and returns the result of the processing step.
# If no function is defined for an instance of an algorithm, the default implementation is called. This method tries to invoke the function `process` with the type of the algorithm:
# ```julia
...
# ```
# The implementation of reconstruction algorithms is thus expected to either implement the `process` function for the algorithm type or instance. Dispatch on instances allow an instance to change its state, while dispatch on types allows for pure helper functions.
...
# Let's define a preprocessing step we can share between our algorithms. We want to allow the user to select certain frames from a time data and average them.
...
# Algorithms can internally construct and use further algorithms to which they pass data.